### ℹ️ About Me (<a href="Resume_2_.pdf" target="_blank">View My CV</a>)
|🏢 I am a **Bachelor of Computer Science (Undergraduate)** at **Edith Cowan University**.|
| :--- |
|💬 Ask me about my experience with **Java, Python, JavaScript, TypeScript, PHP, C#, SQL**, or **Bash**.|
|⚡ I love building dynamic projects using **ReactJS** and exploring **Generative AI**.|
|👨‍💻 Check out my GitHub at **github.com/RasikaRSW** or connect on LinkedIn at **linkedin.com/in/rasikarsw**.|

> I am a Computer Science student with a background in **Software Engineering**. I use my skills in **Java, Python**, and **JavaScript** to design and develop applications. I am passionate about continuous learning and hold numerous certifications in **AI**, **Cybersecurity**, and **Web Development** from platforms like **Google Cloud**, **Meta**, and **Codecademy**.

<br>

### 🎯 &nbsp;My Skills
| 💻 Languages & Frameworks |
| :--- |
| <img src="https://skillicons.dev/icons?i=java,python,js,ts,php,cs,mysql,bash,react,bootstrap,wordpress" />|
| 🛠️ Tools & Design |
| <img src="https://skillicons.dev/icons?i=git,github,figma,ps,pr,ae" />|

<br>

### 🏆 &nbsp;Certificates, Awards and Badges
| 🧠 AI & Machine Learning |
| :--- |
| ☁️ **Google Cloud:** Machine Learning Operations (MLOps) for Generative AI |
| ☁️ **Google Cloud:** Introduction to Generative AI |
| ☁️ **Google Cloud:** Introduction to Large Language Models |
| 🤖 **DeepLearning.AI:** Generative AI for Everyone |
| 🔗 **LinkedIn Learning:** OpenAI API: Fine-Tuning & Fine-Tune Your LLMs |
| 🛡️ **EC-Council:** Generative AI for Cybersecurity |

| 🌐 Web Development & Design |
| :--- |
| ♾️ **Meta:** React Basics & HTML and CSS in depth & Programming with JavaScript |
| 🏕️ **FreeCodeCamp:** Responsive Web Design |
| 🎓 **Coursera:** Build a free website with WordPress |
| 🔗 **LinkedIn Learning:** WordPress: REST API & Intro to Block Themes |
| 💻 **Codecademy:** How to Deploy a Website & Introduction to UI/UX Design |

| 🛡️ Cybersecurity & Core Programming |
| :--- |
| 🔒 **Cisco Networking Academy:** Introduction to Cybersecurity & JavaScript Essentials 1 |
| 🛡️ **EC-Council:** Deep Web and Cybersecurity |
| 💻 **Codecademy:** Learn PHP, Intro to Java, Learn JavaScript, Learn Bash Scripting, Learn the Command Line |
| ⌨️ **HackerRank:** SQL(Basic) |
| 🔗 **LinkedIn Learning:** Practical GitHub Actions |
